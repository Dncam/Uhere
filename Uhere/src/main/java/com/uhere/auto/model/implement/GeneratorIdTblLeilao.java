...
@Component
public class GeneratorIdTblLeilao implements IdentifierGenerator{
	...
	public Serializable generate(SharedSessionContractImplementor session, Object arg1) throws HibernateException {
		...
		try {
	        ...
	        while(true)
	        {
	        	...
	        	ResultSet rs=statement.executeQuery("select count(chave) as chave from uherebd.TBL_LEILAO where chave = '" + newId + "'");
	        	rs.next();
	        		if(rs.getInt(1) == 0)
	        			break;
	        }
